...
func MsgHashWithZ(msg, uid []byte, pubKey *PublicKey) (digest []byte, err error) {
	za, err := ZA(pubKey, uid)
	if err != nil {
		return nil, err
	}
	sm3Hash := sm3.New()
...
func Sm2SignHash(priv *PrivateKey, hash []byte) (r, s *big.Int, err error) {
	e := new(big.Int).SetBytes(hash[:32])
	c := priv.PublicKey.Curve
	N := c.Params().N
	if N.Sign() == 0 {
		return nil, nil, errZeroParam
...
func Sm2Sign(priv *PrivateKey, msg, uid []byte) (r, s *big.Int, err error) {
	hash, err := MsgHashWithZ(msg, uid, &priv.PublicKey)
	if err != nil {
		return nil, nil, err
	}
	r, s, err = Sm2SignHash(priv, hash)
	return
}
