...
var (
	...
	oidNamedCurveP256SM2GM = asn1.ObjectIdentifier{1, 2, 156, 197, 1, 301}   // 早期国密局使用的标识，非10197，是197
)
...
func namedCurveFromOID(oid asn1.ObjectIdentifier) elliptic.Curve {
	...
	case oid.Equal(oidNamedCurveP256SM2) || oid.Equal(oidNamedCurveP256SM2GM):
		return P256Sm2()
	}
	return nil
}
	...
	for _, e := range in.TBSCertificate.Extensions {
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 19:
				...
				// 8字节，兼容非标准的基本约束  Tag len Tag len 00/0xff Tag len val
				// DER demands that "If the encoding represents the boolean value TRUE,
				// its single contents octet shall have all eight bits set to one."
				// Thus only 0 and 255 are valid encoded values.
				var constraintsBytes = e.Value
				if len(constraintsBytes) >= 5 && (constraintsBytes[4] != 0 && constraintsBytes[4] != 0xff) {
					constraintsBytes[4] = 0xff
				}

				if rest, err := asn1.Unmarshal(constraintsBytes, &constraints); err != nil {
					return nil, err
				} else if len(rest) != 0 {
...
// All keys types that are implemented via crypto.Signer are supported (This
// includes *rsa.PublicKey and *ecdsa.PublicKey.)
func CreateCertificateRequestWithNullSignParam(rand io.Reader, template *CertificateRequest, priv interface{}, withNullSignParam bool) (csr []byte, err error) {
	key, ok := priv.(crypto.Signer)
	if !ok {
		return nil, errors.New("x509: certificate private key does not implement crypto.Signer")
	}
	...
	switch template.SignatureAlgorithm {
	case SM2WithSM3:
		if priKeyObj, ok := priv.(*PrivateKey); ok {
			r, s, err := Sm2Sign(priKeyObj, tbsCSRContents, []byte("1234567812345678"))
			if err != nil {
				return nil, err
			}
			signature, _ = SignDigitToSignData(r, s)
		} else {
			return nil, errors.New("create CSR error! sm2 sign not supported key type " + reflect.TypeOf(priv).String())
		}
		break
	default:
		h := hashFunc.New()
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv interface{}) (csr []byte, err error) {
	csr, err = CreateCertificateRequestWithNullSignParam(rand, template, priv, false)
	return
}
